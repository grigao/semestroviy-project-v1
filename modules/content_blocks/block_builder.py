...
class ContentBlockBuilder:
    ...
    def __init__(self, token_model: str = "cl100k_base", skip_patterns: Optional[List[str]] = None):
    ...
    def estimate_tokens(self, text: str) -> int:
        """Считает точное количество токенов с помощью загруженного энкодера."""
        return len(self.encoder.encode(text))

    def is_large_block(self, text: str) -> bool:
        """Проверяет, превышает ли блок лимиты по символам или токенам."""
    ...
    def _should_skip_block(self, text: str) -> bool:
        """Проверяет, содержит ли блок стоп-слова (без учета регистра)."""
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        for block in blocks:
            ...
            if self.is_large_block(block):
                ...
                result.extend([s.strip() for s in sentences if s.strip() and not self._should_skip_block(s.strip())])
            else:
                result.append(block)
    ...
    def process_archive(self, input_dir: Path, output_dir: Optional[Path] = None) -> List[Path]:
        ...
        for json_file in json_files:
            ...
            for message in messages:
                ...
                file_skipped += old_blocks_count - new_blocks_count if old_blocks_count > new_blocks_count else 0
                
                if settings.debug and new_blocks_count != old_blocks_count:
                    logger.debug("message.blocks id=%s blocks=%d skipped=%d", 
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                json.dump({"metadata": metadata, "messages": messages}, f, ensure_ascii=False, indent=2)
            
            logger.info("file.complete name=%s messages=%d blocks=%d skipped=%d", 
                    json_file.name, len(messages), file_blocks, file_skipped)
            output_paths.append(output_path)
            
            total_blocks += file_blocks
            total_skipped += file_skipped
        
        logger.info("blocks.complete files=%d total=%d blocks=%d skipped=%d", 
